...
Given(/^A valid "([^"]+)" request payload is prepared$/, async function (operationName) {
...
When(/^The payload is made invalid by removing "([^"]+)"$/, async function (field) {
  const payload = clone(this.payload);
  delete payload[field];

  let didFail = false;
  try {
    await assertOpenApiRequest({ path: getRequestPath(this.endpoint), method: 'post' }, payload);
  } catch {
    didFail = true;
  }
...
When(/^The invalid "([^"]+)" request is sent$/, async function (operationName) {
  const op = getOperationDef(operationName);
...
Then(/^The implementation should reject the invalid request$/, async function () {
