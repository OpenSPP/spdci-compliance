@profile=crvs-subscriber @tier=core
Feature: CRVS Subscriber rejects schema-invalid callback requests

These scenarios verify CORE error-handling for CRVS subscriber endpoints.
Acceptable behaviors:
- HTTP 4xx with HttpErrorResponse, OR
- HTTP 200 with Response where message.ack_status = ERR

  @smoke @method=POST @endpoint=registry/on-search @req=CRVS-CORE-SP-ON-SEARCH-NEG-01
  ...
  @smoke @method=POST @endpoint=registry/on-subscribe @req=CRVS-CORE-SP-ON-SUBSCRIBE-NEG-01
  ...
  @smoke @method=POST @endpoint=registry/on-unsubscribe @req=CRVS-CORE-SP-ON-UNSUBSCRIBE-NEG-01
  ...
  @smoke @method=POST @endpoint=registry/txn/on-status @req=CRVS-CORE-SP-TXN-ON-STATUS-NEG-01
  ...
  @smoke @method=POST @endpoint=registry/notify @req=CRVS-CORE-SP-NOTIFY-NEG-01
