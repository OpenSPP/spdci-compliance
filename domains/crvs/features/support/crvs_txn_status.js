...
Given(/^System wants to check transaction status in CRVS$/, function () {
  specTxnStatus = spec();
});

When(/^A POST request to CRVS txn status is sent$/, async function () {
  try {
    applyCommonHeaders(specTxnStatus.post(baseUrl));
    const payload = createTxnStatusRequestPayload();
    await assertOpenApiRequest({ path: getRequestPath(asynctxnstatusEndpoint), method: 'post' }, payload, 'crvs');
...
Then(/^The response from the CRVS txn status should be received$/, async function () {
  chai.expect(this.response).to.exist;
});

Then(/^The CRVS txn status response should have status (\d+)$/, async function(status) {
  chai.expect(this.response.statusCode).to.equal(Number(status));
});

Then(/^The CRVS txn status response should have status (\d+) or (\d+)$/, async function(statusA, statusB) {
...
Then(/^The CRVS txn status response should have "([^"]*)": "([^"]*)" header$/, async function(key, value) {
...
Then(/^The CRVS txn status response should be returned in a timely manner$/, async function() {
  chai.expect(this.response.responseTime).to.be.lessThan(defaultExpectedResponseTime);
});

Then(/^The CRVS txn status response should match the expected JSON schema$/, async function() {
