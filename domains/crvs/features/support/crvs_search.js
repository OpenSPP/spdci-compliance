...
Given(/^System wants to async search for person in CRVS$/, function () {
  specAsyncSearch = spec();
});

When(/^A POST request to CRVS async search is sent$/, async function () {
  try {
...
Then(/^The response from the CRVS async search should be received$/, async function () {
  chai.expect(this.response).to.exist;
});

Then(/^The CRVS async search response should have status (\d+)$/, async function(status) {
  chai.expect(this.response.statusCode).to.equal(Number(status));
});

Then(/^The CRVS async search response should have status (\d+) or (\d+)$/, async function(statusA, statusB) {
...
Then(/^The CRVS async search response should have "([^"]*)": "([^"]*)" header$/, async function(key, value) {
...
Then(/^The CRVS async search response should be returned in a timely manner$/, async function() {
  chai.expect(this.response.responseTime).to.be.lessThan(defaultExpectedResponseTime);
});

Then(/^The CRVS async search response should have ack_status ACK$/, async function() {
  chai.expect(this.response.body?.message?.ack_status).to.equal('ACK');
});

Then(/^The CRVS async search response should match the expected JSON schema$/, async function() {
