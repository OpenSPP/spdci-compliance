...
import { waitForCallback } from '../../../../common/helpers/callback-server.js';
...
async function assertCallbackMatchesSpec(path, record) {
  await assertOpenApiRequest({ path, method: 'post', domain: 'crvs' }, record.body);
}
...
async function sendAsyncRequest(world, { action, endpoint, payload }) {
  ...
  await assertOpenApiRequest({ path: requestPath, method: 'post' }, payload, 'crvs');
...
When(/^SP sends an async search request to CRVS expecting a callback$/, async function () {
...
When(/^SP sends an async subscribe request to CRVS expecting a callback$/, async function () {
  await sendAsyncRequest(this, {
    action: 'subscribe',
...
When(/^SP sends an async unsubscribe request to CRVS expecting a callback$/, async function () {
  await sendAsyncRequest(this, {
    action: 'unsubscribe',
...
When(/^SP sends an async txn status request to CRVS expecting a callback$/, async function () {
  await sendAsyncRequest(this, {
    action: 'txn-status',
...
Then(/^CRVS should respond with ACK for the async request$/, async function () {
  ...
  await assertOpenApiResponse({ path: requestPath, method: 'post', statusCode: response.statusCode }, response.body, 'crvs');
...
Then(/^CRVS should call the on-search callback with matching ids$/, async function () {
...
Then(/^CRVS should call the on-subscribe callback with matching ids$/, async function () {
  const { payload, response } = this.workflow;
  const expectedTxn = payload?.message?.transaction_id;
  const expectedCorr = getAckCorrelationId(response.body);
...
Then(/^CRVS should call the on-unsubscribe callback with matching ids$/, async function () {
  const { payload, response } = this.workflow;
  const expectedTxn = payload?.message?.transaction_id;
  const expectedCorr = getAckCorrelationId(response.body);
...
Then(/^CRVS should call the txn on-status callback with matching ids$/, async function () {
  const { payload, response } = this.workflow;
  const expectedTxn = payload?.message?.transaction_id;
  const expectedCorr = getAckCorrelationId(response.body);
