...
Given(/^System wants to unsubscribe from CRVS events$/, function () {
  specUnsubscribe = spec();
});

When(/^A POST request to CRVS unsubscribe is sent$/, async function () {
  try {
    applyCommonHeaders(specUnsubscribe.post(baseUrl));
    const payload = createUnsubscribeRequestPayload();
    await assertOpenApiRequest({ path: getRequestPath(unsubscribeEndpoint), method: 'post' }, payload, 'crvs');
...
Then(/^The response from the CRVS unsubscribe should be received$/, async function () {
  chai.expect(this.response).to.exist;
});

Then(/^The CRVS unsubscribe response should have status (\d+)$/, async function(status) {
  chai.expect(this.response.statusCode).to.equal(Number(status));
});

Then(/^The CRVS unsubscribe response should have status (\d+) or (\d+)$/, async function(statusA, statusB) {
...
Then(/^The CRVS unsubscribe response should have "([^"]*)": "([^"]*)" header$/, async function(key, value) {
...
Then(/^The CRVS unsubscribe response should be returned in a timely manner$/, async function() {
  chai.expect(this.response.responseTime).to.be.lessThan(defaultExpectedResponseTime);
});

Then(/^The CRVS unsubscribe response should have ack_status ACK or NACK$/, async function() {
  const ackStatus = this.response.body?.message?.ack_status;
  chai.expect(['ACK', 'NACK']).to.include(ackStatus);
});

Then(/^The CRVS unsubscribe response should match the expected JSON schema$/, async function() {
