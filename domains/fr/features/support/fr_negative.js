...
Given(/^A valid FR "([^"]+)" request payload is prepared$/, async function (operationName) {
...
When(/^The FR payload is made invalid by removing "([^"]+)"$/, async function (field) {
  const payload = clone(this.payload);
  delete payload[field];

  let didFail = false;
  try {
    await assertOpenApiRequest({ path: getRequestPath(this.endpoint), method: 'post' }, payload, 'fr');
  } catch {
    didFail = true;
  }
...
When(/^The invalid FR "([^"]+)" request is sent$/, async function (operationName) {
  const op = getOperationDef(operationName);
...
Then(/^The FR implementation should reject the invalid request$/, async function () {
  ...
  if (status >= 400 && status < 500) {
    ...
    await assertOpenApiComponentResponse('Response', this.response.body, 'fr');
