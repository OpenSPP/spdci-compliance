...
Given(/^System wants to sync search for farmer in FR$/, function () {
  specSearch = spec();
});

When(/^A POST request to FR sync search is sent$/, async function () {
  try {
...
Then(/^The response from the FR sync search should be received$/, async function () {
  chai.expect(this.response).to.exist;
});

Then(/^The FR sync search response should have status (\d+)$/, async function(status) {
  chai.expect(this.response.statusCode).to.equal(Number(status));
});

Then(/^The FR sync search response should have status (\d+) or (\d+)$/, async function(statusA, statusB) {
...
Then(/^The FR sync search response should have "([^"]*)": "([^"]*)" header$/, async function(key, value) {
...
Then(/^The FR sync search response should be returned in a timely manner$/, async function() {
  chai.expect(this.response.responseTime).to.be.lessThan(defaultExpectedResponseTime);
});

Then(/^The FR sync search response should match the expected JSON schema$/, async function() {
