...
Given(/^System wants to check sync transaction status in FR$/, function () {
...
When(/^A POST request to sync txn status is sent$/, async function () {
  ...
  await assertOpenApiRequest({ path: getRequestPath(txnstatusEndpoint), method: 'post' }, payload, 'fr');
